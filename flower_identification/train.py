...
# 数据集地址
path='E:/python/PycharmProject/flower_identification/data/flowers/'
# 模型保存地址
model_path='E:/python/PycharmProject/flower_identification/model/model.ckpt'
# 模型可视化地址（TensorBoard查看）
graph_path='E:/python/PycharmProject/flower_identification/graph'
...
def inference(input_tensor, train, regularizer):
    ...
    with tf.variable_scope('layer1-conv1'):
        # 初始化权重conv1_weights，大小5*5 三通道（RGB），数量32
        conv1_weights = tf.get_variable("weight",[5,5,3,32],initializer=tf.truncated_normal_initializer(stddev=0.1))
        # 初始化偏置conv1_biases，数量32
        conv1_biases = tf.get_variable("bias", [32], initializer=tf.constant_initializer(0.0))
        # 设置上下左右滑动步数为1，输入输出一致
        conv1 = tf.nn.conv2d(input_tensor, conv1_weights, strides=[1, 1, 1, 1], padding='SAME')
        # 激励计算 ，调用tensorflow 的relu函数
        relu1 = tf.nn.relu(tf.nn.bias_add(conv1, conv1_biases))

    with tf.name_scope("layer2-pool1"):
        # 池化计算 ，调用tensorflow 的max_pool函数
    ...
    with tf.variable_scope('layer9-fc1'):
        ...
        # 采用dropout层，减少过拟合和欠拟合程度，保存模型最好的预测效率
...
# 绘制训练精度验证精度图像
plt.plot(epoch, fig_acc1, 'bo', label='Training acc')
plt.plot(epoch, fig_acc2, 'b', label='Validation acc')
...
# 绘制训练损失，验证损失图像
plt.plot(epoch, fig_loss1, 'bo',  label='Training loss')
plt.plot(epoch, fig_loss2, 'b', label='Validation loss')
